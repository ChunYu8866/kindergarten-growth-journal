...
畫布不是以 1:1 貼上這些素材，而是放大後置中再裁掉超出的部分，讓裝飾留在外圍：春日 ×1.24、四季 ×1.20、森林 ×1.18 並下移 5%、海洋 ×1.20 並下移 1%、彩虹 ×1.20、恐龍 ×1.18 並下移 2%、農場 ×1.18 並下移 2%、太空 ×1.16、玩具 ×1.18 並下移 1%。這些倍率定義在 `app/page.tsx` 的 `DECORATIONS`，版型縮圖用同一組值做 CSS transform。有使用裝飾的版型，其成品圖層依序為底色／紋理、滿版照片、透明裝飾，因此裝飾會自然疊在照片外緣；無裝飾版型不載入這些素材。
